...
export default function ProductGallery({ images, title, primaryImage }: ProductGalleryProps) {
    ...
    return (
        <div className="flex flex-col md:flex-row gap-4 h-full">
            ...
            {/* Main Image */}
            <div className="relative flex-1 bg-white border border-[var(--border)] rounded-md aspect-square sm:aspect-auto sm:h-[480px] lg:h-[520px] flex items-center justify-center overflow-hidden order-1 md:order-2">
                {mainImage ? (
                    <Image
                        src={mainImage}
                        alt={title}
