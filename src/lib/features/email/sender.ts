...
export async function sendPriceDropEmail(data: PriceDropData) {
    ...
    const percentageDrop = Math.round(((data.oldPrice - data.newPrice) / data.oldPrice) * 100);
            ...
            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                ...
                <div style="padding: 40px 30px;">
                    ...
                    <div style="background-color: #f3f4f6; border-radius: 8px; padding: 20px; margin: 30px 0; text-align: center;">
                        ${data.productImage ? `<img src="${data.productImage}" alt="Product" style="max-width: 150px; max-height: 150px; object-fit: contain; margin-bottom: 20px;" />` : ''}
                        <h3 style="color: #111827; margin: 0 0 15px 0; font-size: 18px;">${data.productTitle}</h3>
                        
                        <div style="display: flex; justify-content: center; align-items: center; gap: 15px; margin-bottom: 10px;">
                            <span style="color: #9ca3af; text-decoration: line-through; font-size: 16px;">₹${data.oldPrice.toLocaleString()}</span>
                            <span style="color: #10b981; font-weight: 800; font-size: 24px;">₹${data.newPrice.toLocaleString()}</span>
                        ...
                        <p style="color: #6b7280; margin: 0; font-size: 14px;">Your Target: ₹${data.targetPrice.toLocaleString()}</p>
                    </div>

                    <div style="text-align: center; margin-top: 30px;">
                        <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/products/${data.productSlug}" 
